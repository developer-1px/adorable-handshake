...
const makeAdorableValue = (value: string) => {
  return value
    ...
    .replace(/#([0-9a-fA-F]{4,8})/g, (_, hex) => {
      ...
      if (hex.length === 8)
        return "#" + shortHex(hex.slice(0, 6)) + (parseInt(hex.slice(6), 16) / 255).toFixed(2).replace(/0$/, "").slice(1)
  ...
  (node: FrameNode, cls: Record<string, string> = {}) => {
    ...
    function transform(prop: string, value: string | number): [string, string?] {
      ...
      switch (prop) {
        ...
        case "flex": {
          if (node.parent.layoutMode === "HORIZONTAL") return ["w", "fill"]
          if (node.parent.layoutMode === "VERTICAL") return ["h", "fill"]
          return ["flex", value]
        }
        case "align-self": {
          if (node.parent.layoutMode === "HORIZONTAL") return ["h", "fill"]
          if (node.parent.layoutMode === "VERTICAL") return ["w", "fill"]

...
const prefixAddOne = (str) => str.replace(/(\d*)$/, (_, p1) => Number(p1) + 1)
...
const addClassBorder = (node, addClass:AddClass) => {
  ...
  if (typeof strokeWeight === "number" && strokeWeight >= 0) {
    ...
    addClass("border", `${px(strokeWeight)} solid ${makeColor(border.color, border.opacity)}`)
  }
  else {
    addClass("border-color", makeColor(border.color, border.opacity))
    addClass("border-style", "solid")
    ...
    if (strokeTopWeight) addClass("border-top-width", px(strokeTopWeight))
    if (strokeRightWeight) addClass("border-right-width", px(strokeRightWeight))
    if (strokeBottomWeight) addClass("border-bottom-width", px(strokeBottomWeight))
    if (strokeLeftWeight) addClass("border-left-width", px(strokeLeftWeight))
  }
}
  ...
  node.effects.forEach(effect => {
    ...
    switch (effect.type) {
      ...
      case "INNER_SHADOW": {
        ...
        addClass("box-shadow", [inset, x, y, radius, spread, makeColor(color, color.a)].filter(Boolean).join(" "))
        break
      }
...
const addClassFlexbox = (node:FrameNode, addClass:AddClass) => {
  ...
  if (layoutMode !== "NONE" && hasChildren) {
    ...
    if (layoutMode === "HORIZONTAL") {
      ...
      addClass("flex-flow", ["row", layoutWrap === "WRAP" && "wrap"].filter(Boolean).join(" "))
    }

    // vbox
    else if (layoutMode === "VERTICAL") {
      addClass("display", "flex")
      addClass("flex-flow", ["column", layoutWrap === "WRAP" && "wrap"].filter(Boolean).join(" "))
    }
...
export const getGeneratedCode = (node:SceneNode) => {
  const {init, root} = createClassBuilder()
  init()
